(ns marketentry.marketentryllm
  ...
  the HRV public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real EOJN RH submission. Every
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world EOJN RH submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "requires-foreign-investment-approval?=" (:requires-foreign-investment-approval? e)
                        " foreign-investment-approval-verified?=" (:foreign-investment-approval-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-foreign-investment-approval? e))
                              (:foreign-investment-approval-verified? e)))
                   0.9 0.3)}))
