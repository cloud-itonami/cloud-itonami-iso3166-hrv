  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems HRV"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems HRV" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest foreign-investment-approval-missing-is-held-and-unoverridable
  (testing "missing verified Croatian FDI-screening approval for an engagement that requires one -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:foreign-investment-approval-missing} (-> (store/ledger db) last :basis)))
