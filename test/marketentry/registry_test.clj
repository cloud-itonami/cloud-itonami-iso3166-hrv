...
(deftest register-draft-and-submit
  (let [d (registry/register-draft "eng-1" "HRV" 0)
        s (registry/register-submit "eng-1" "HRV" 0)]
    (is (= "HRV-DFT-000000" (get d "draft_number")))
    (is (= "HRV-SUB-000000" (get s "submit_number")))
    (is (nil? (get-in d ["certificate" "proof"])))
    (is (= "draft-unsigned" (get-in s ["certificate" "status"])))))

(deftest register-requires-ids
  (is (thrown? Exception (registry/register-draft "" "HRV" 0)))
